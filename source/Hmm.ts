...
export abstract class Hmm<State, Symbol> {
    ...
    public abstract viterbi(s: Array<Symbol>): Array<State>
    ...
    constructor(states: Set<State>, observations: Array<Array<State>>, emittedSymbols: Array<Array<Symbol>>) {
        ...
        this.states = new Array<HmmState<State, Symbol>>()
        this.stateIndexes = new Map<State, number>()
     ...
     */
    calculateEmissionProbabilities(state: State, observations: Array<Array<State>>,
                                   emittedSymbols: Array<Array<Symbol>>): Map<Symbol, number>{
        let counts = new CounterHashMap()
        let emissionProbabilities = new Map<Symbol, number>()
        ...
        for (let symbol of counts.keys()){
            emissionProbabilities.set(<Symbol>symbol, counts.get(symbol) / sum)
        }
